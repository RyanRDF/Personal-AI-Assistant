...
export class PersonalAssistant {
  ...
  async reply(chatId: string, userText: string): Promise<string> {
    this.dependencies.conversations.add(chatId, "user", userText);
    ...
    for (let iteration = 0; iteration < 6; iteration += 1) {
      ...
      if (!responseMessage) throw new Error("Model tidak mengembalikan respons.");
      messages.push(responseMessage);

      const calls = responseMessage.tool_calls ?? [];
      if (calls.length === 0) {
        const answer = responseMessage.content?.trim() || "Maaf, saya belum dapat menjawabnya.";
